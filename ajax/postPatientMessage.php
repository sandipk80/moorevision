...
//post message
if(isset($_POST['message'],$_POST['conversation_id'],$_POST['doc_id']) && trim($_POST['conversation_id'])!=="" && trim($_POST['doc_id'])!==""){
	$filename = "";
	$attachname = "";
	$filetype = "";
	$filesize = "0";
	...
	$msgArray = array(
		...
		'user_id' => $userId,
		'doctor_id' => base64_decode($_POST['doc_id']),
		'msg_from' => 'U',
	...
	if($saveMessage){
		...
		$getDoctor = $globalManager->runSelectQuery("doctors", "first_name,last_name,email", "id='".base64_decode($_POST['doc_id'])."'");
		if(is_array($getDoctor) && !empty($getDoctor)){
			$doctorName = ucwords($getDoctor[0]['first_name'].' '.$getDoctor[0]['last_name']);
			$doctorEmail = $getDoctor[0]['email'];
			$message = 'Dear '.$doctorName;
			$message .= '<p>You just receive a message from patient on Moore Vision. Below is your message:</p>';
			$message .= '<p>&nbsp;</p>';
			...
			$phpmailer->AddAddress($doctorEmail, $doctorName);
