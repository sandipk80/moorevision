...
if(is_array($_REQUEST) && count($_REQUEST)>0) {
    $result = $_REQUEST;
    if(isset(isset($result['doctor_id'],$result['doctor_key'],$result['patient_id'],$result['examination_id'],$result['action']) && trim($result['doctor_id'])!=='' && trim($result['doctor_key'])!=='' && trim($result['patient_id'])!=='' && trim($result['examination_id'])!=="" && trim($result['action'])=='edit_patient_examination') {
        ...
        ##---------------CHECK EXAMINATION IN DATABASE START------------------##
        $exWhere = "id='".$result['examination_id']."' AND user_id='".$result['patient_id']."'";
        $getUserRecord = $globalManager->runSelectQuery('patient_examinations','IFNULL(COUNT(id),0) as total',$exWhere);
        if(is_array($getUserRecord[0]['total']) && $getUserRecord[0]['total']>0) {
        }
        else {
            header('Content-Type: application/json');
            $arr = array('status'=>'error','message'=>'Patient examination not exists in our database');
            echo json_encode($arr); die;
        }
		...
		$updateExamination = $globalManager->runUpdateQuery("patient_examinations", $examArray, "id='".$result['examination_id']."' AND user_id='".$result['patient_id']."'");
		if($updateExamination){
			header('Content-Type: application/json');
            $arr = array('status'=>'success','message'=>'Patient examination details has been updated.');
            echo json_encode($arr); die;
        }else{
            header('Content-Type: application/json');
